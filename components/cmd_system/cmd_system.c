...
static void register_app_description(void);
...
static int get_application_description(int argc, char **argv)
{
    ...
    printf("Application SHA: ");
    for (int i = 0; i < 32; i++) {
        printf("%02x", app_desc->app_elf_sha256[i]);
    }
    printf("\n");
...
static void register_app_description(void)
{
    const esp_console_cmd_t cmd = {
        .command = "app_desc",
        .help = "Get application description",
        .hint = NULL,
        .func = &get_application_description,
    };
    ESP_ERROR_CHECK( esp_console_cmd_register(&cmd) );
}
